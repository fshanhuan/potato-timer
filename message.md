- - 后端对接说明（2026-05-29 更新，最后更新：用户头像功能）

    后端文件：

    - `PomodoroTimer.py` — 核心业务层
    - `ai_assistant.py` — AI 自然语言解析模块
    ...
    **十二、用户头像（新增）**
    ...
    每个用户可设置一个本地头像，头像文件存储在 `local_user_data/avatars/` 目录下。

    **设置头像：**

    ```python
    path = user.set_avatar("/path/to/photo.png")
    ```

    规则：

    - 源文件必须是本地图片，支持格式：`.png`、`.jpg`、`.jpeg`、`.gif`、`.webp`、`.bmp`
    - 文件大小限制 5MB
    - 不符合条件时抛出 `ValueError`，前端应捕获并提示用户
    - 头像自动复制到 `local_user_data/avatars/{安全昵称}_avatar{ext}`
    - 更换头像时自动删除旧文件（包括不同扩展名的旧头像）
    - 设置后自动持久化，重启后自动恢复

    **获取头像：**

    ```python
    path = user.get_avatar_path()  # 返回 Path 或 None（头像不存在/已删除时）
    ```

    规则：

    - 返回 `Path` 对象或 `None`
    - 即使 `avatar_path` 有值，如果文件已被手动删除也会返回 `None`
    ...
    **用户序列化中的头像：**
    ...
    `user.to_dict()` 返回的字典中包含 `avatar_path` 字段：

    ```json
    {
      "user_id": 123,
      "username": "小明",
      "avatar_path": "C:/.../local_user_data/avatars/小明_avatar.png",
      ...
    }
    ```
    ...
    **前端注意：**
    ...
    - 设置/删除头像不会返回完整用户对象，前端如需更新 UI 可调用 `user.to_dict()` 重新获取
    - 头像路径是绝对路径；前端展示时可直接读取本地文件或转为 base64 / file URL
    ...
    **十三、建议 API 设计**
    ...
    POST /users/{nickname}/avatar        ← 上传头像（新增）
    DELETE /users/{nickname}/avatar       ← 删除头像（新增）
    GET  /users/{nickname}/avatar         ← 获取头像路径（新增）
    ...
    **十四、前端特别注意**
    ...
    - **所有用户数据均已持久化**（2026-05-29 更新）：
      ...
      - 用户头像 → `avatars/{昵称}_avatar{ext}`，设置/删除时自动保存。
